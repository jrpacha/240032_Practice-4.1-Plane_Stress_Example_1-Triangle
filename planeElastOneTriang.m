...
%Computation of the stiff Matrix
...
%Fm=F(freeNod)-K(freeNod,fixedNod)*u(fixedNod); %Here u(fixedNod)=0,
                                                %so this is not necessary
                                                
%Fm=Fm+Q(freeNod); %F=0: no "internal forces(e.g. the body weight)
                   %are taken into account. So we can set, directly
                   %Fm=Q(freeNod)
Fm=Q(freeNod);
